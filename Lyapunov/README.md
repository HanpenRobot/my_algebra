...
+ Logistic 写像$`(1)`$の初期値鋭敏性を数値計算で確認する。
```math
x_{n+1}=f(x_{n})=4x_{n}(1-x_{n}) \cdots (1)
```

+ 初期値を$`𝑥_{0}=0.1`$，$`𝑥_{0}=0.10001`$とした時の計算結果を Fig. 1 に示す.
+ $`n \le 10`$では2つの結果はほぼ一致しているが，$`n>11`$になると差が大きくなっていることが確認できる。以上よりLogistic 写像の初期値鋭敏性が確認できた.
+ Fig. 1 のグラフの作成に使用したPythonプログラムは[./logistic_map.py](./logistic_map.py)である。

![./logistic_plot.png](./logistic_plot.png)
*Fig. 1 Logistic 写像の初期値鋭敏性*


## Logistic 写像の Lyapunov 指数 $`\lambda`$
+ Logistic 写像 $`(1)`$のLyapunov 指数 $`\lambda`$を$`(2)`$で定義する。
```math
\lambda = \lim_{T\to\infty} \frac{1}{T} \sum^{T-1}_{n=0}{\log\left| \frac{df({x_n})}{dx_n}\right| } = \lim_{T\to\infty} \frac{1}{T} \sum^{T-1}_{n=0}{\log\left| 4(1-2x_{n})\right| }\cdots (2)
```
...
+ Logistic 写像 $`(1)`$の不変分布 $`\rho(x)`$を$`(3)`$で定義する。
```math
\rho(x) = \lim_{T\to\infty} \frac{1}{T} \sum^{T-1}_{n=0}{\delta(x-x_n)  }\cdots (3)
```
