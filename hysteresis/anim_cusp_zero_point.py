...
def draw(n: int):
    print(f"plotting {n=} ...")
    # A = [-2.0]
    A = list(np.arange(0, 5, 0.1))
    x_h = np.sqrt(A[n] / 3.0)
    cusp_value = cusp(x=x_h, A=A[n])
    plt.cla()
    plt.title(
        f"A=${A[n]:.2f}, B={cusp_value:.4f}$",
    ...
    for x in range(N):
        ...
        Y.append(cusp(x=start_value, A=A[n]) + cusp_value)
        Y2.append(cusp(x=start_value, A=A[n]) - cusp_value)

        start_value += step_value
    plt.plot(
        X,
        Y,
        label=r"$y=-x^3+Ax+\sqrt{ \frac {A}{3.0} }$",
        color="#0000FF",
...
res.save(f"./new_cusp_anim_zeropoint.gif", writer="imagemagick")
