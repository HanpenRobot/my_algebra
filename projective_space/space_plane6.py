...
def create_frame(angle: int):
    ...
    y = TMP_ARRY
    X, Y = np.meshgrid(x, y)
    ax.plot_surface(X, Y, Z=X * 0 + 1.0, color="#AA0000", label=f"$Z=1$", alpha=0.3)
    ax.plot_surface(X, Y, Z=X * 0 + 3.0, color="#0000FF", label=f"$Z=3$", alpha=0.2)
    ...
    tmp_X = [-tmp_value, tmp_value]
    tmp_Y = [-(tmp_value**2), tmp_value**2]
    tmp_Z = [-1, 1]
