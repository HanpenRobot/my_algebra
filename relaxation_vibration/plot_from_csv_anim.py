...
def get_pos(ary: int, frames: int, n: int):
    step_size = len(ary) / frames
    pos = int(step_size * n)
    return pos
...
def draw(n: int):
    ...
    TITLE_FONT_SIZE = 14
    LABEL_FONT_SIZE = 16
    pos = get_pos(ary=ans_V, frames=FRAMES, n=n)
    plt.cla()
    ...
    plt.plot(ans_T[:pos], ans_V[:pos], "o", ms=1, color=RT_LINE_COLOR)
...
def draw2(n: int):
    ...
    plt.plot(ans_T[:pos], ans_X[:pos], "o", ms=1, color=RT_LINE_COLOR)
    # plt.legend(loc="lower center", borderaxespad=1, fontsize=10)
...
def draw3(n: int):
    ...
    plt.plot(ans_X[:pos], ans_V[:pos], "o", ms=1, color=RT_LINE_COLOR)
    plt.plot(
        ans_X[pos - 2 : pos], ans_V[pos - 2 : pos], "o", ms=3, color=CURVE_LINE_COLOR
    )
    # plt.legend(loc="lower center", borderaxespad=1, fontsize=10)


res = FuncAnimation(fig, draw, interval=50, frames=range(FRAMES))
res.save(f"relax_map_v_t_anim.gif", writer="imagemagick")

res = FuncAnimation(fig, draw2, interval=50, frames=range(FRAMES))
res.save(f"relax_map_x_t_anim.gif", writer="imagemagick")


res = FuncAnimation(fig, draw3, interval=50, frames=range(FRAMES))
res.save(f"relax_map_x_v_anim.gif", writer="imagemagick")
