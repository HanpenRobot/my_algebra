...
double func1(double a, double x, double y)
{
    return -x * (pow(x, 2) + a * pow(y, 2));
}

double func2(double a, double x, double y)
{
    return -y * (pow(y, 2) + a * pow(x, 2));
    // if (2.0 * y > x)
    // {
    //     return 0.2 * y - 0.5 * x;
    // }
...
double run_runge_result(FILE *fp, double a, double x0, double y0, int eq_num)
{
    ...
    {
        k1 = h * func1(a, x, y);
        k2 = h * func1(a, x, y + l1 / 2.0);
        k3 = h * func1(a, x, y + l2 / 2.0);
        k4 = h * func1(a, x, y + l3);
...
int main(void)
{
    FILE *fp = fopen("ans_sliding_mode.csv", "w");
    fprintf(fp, "t,x,y,z\n");
    double a = 5.0;

    int eq_num = 0;
    for (double x0 = -0.3; x0 < 0.3; x0 += 0.1)
        for (double y0 = -0.3; y0 < 0.3; y0 += 0.1)
        {
            {
