...
*Fig. 2 重複固有値をもつ線形ベクトル場を4次のルンゲ・クッタ法で解いた結果のアニメーション*

+ 固有値が共役複素数になる線形ベクトル場を$`(5)`$,$`(6)`$で定義する。
```math
\frac{du}{dt}=au-bv \cdots (5)
```

```math
\frac{dv}{dt}=bv+av \cdots (6)
```

![./vec_field_ans_jordan_flow3.gif](./vec_field_ans_jordan_flow3.gif)
*Fig. 3 固有値が共役複素数になる線形ベクトル場を4次のルンゲ・クッタ法で解いた結果のアニメーション*
