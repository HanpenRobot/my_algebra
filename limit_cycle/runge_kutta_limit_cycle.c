...
double func1(double a, double b, double x, double y)
{
    ...
    return a * x - b * y - x * (pow(x, 2) + pow(y, 2));
}

double func2(double a, double b, double x, double y)
{

    return a * y + b * x - y * (pow(x, 2) + pow(y, 2));
}
...
double run_runge_result(FILE *fp, double a, double b, double x0, double y0, int eq_num)
{
    ...
    {
        k1 = h * func1(a, b, x, y);
        k2 = h * func1(a, b, x, y + l1 / 2.0);
        k3 = h * func1(a, b, x, y + l2 / 2.0);
        k4 = h * func1(a, b, x, y + l3);
...
int main(void)
{
    FILE *fp = fopen("ans_limit_cycle.csv", "w");
    fprintf(fp, "t,x,y,z\n");
    double a = 25.0;
    double b = 5.0;
    ...
    for (double x0 = -10.0; x0 < 10.0; x0 += 0.5)
        for (double y0 = -10.0; y0 < 10.0; y0 += 0.5)
        {
            {
