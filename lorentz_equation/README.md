...
+ 下記のLorentz方程式を$`\sigma = 10, R = 28, b = \frac{8}{3}`$、初期値$`x(0)=0.3, y(0)=0, z(0)=0`$の条件で4次のルンゲ・クッタ法で解き、その結果を3Dプロットした。微分方程式を解く際に使用したルンゲ・クッタ法のコードは[./runge_kutta_3d.c](./runge_kutta_3d.c)である。 (このコードは参考文献[1]のコードを参考に実装した)。
```math
