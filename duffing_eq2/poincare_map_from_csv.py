...
# なので、2pi / 0.0005 = 3.14 / 0.0005 = 6280 毎にx-yをよぎる点がポアンカレ写像の点
...
def create_frame(num: int):
    ...
    plt.ylabel(r"$x(t)$", fontsize=LABEL_FONT_SIZE)
    ...
    # Duffing方程式の解曲線をプロット
    plt.plot(ans_T, ans_X, color=X1_LINE_COLOR, linewidth=1)
    plt.title(rf"Poincare's map", fontsize=12, loc="center")
    plt.close()
