...
![./vec_field_zz-1.gif](./vec_field_zz-1.gif)
...
![./vec_field_zzz-1.gif](./vec_field_zzz-1.gif)
...
![./vec_field_z+zbar.gif](./vec_field_z+zbar.gif)
*Fig. 3 $`z + \frac{9}{z}`$のベクトル場 ($`z=x+iy`$を代入して$`w`$を求め、各$`(x,y)`$に$`w`$をベクトルとみなしてプロットした結果)*

Fig. 3は[./draw_vector_field3.py](./draw_vector_field3.py)のコードで作成した。
