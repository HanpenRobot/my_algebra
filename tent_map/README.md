...
## 区分線形の対称型テント写像の不変分布
+ テント写像$`x_{n+1}=T(x_n)`$の不変分布$`\rho(x)`$が一様分布になることを数値計算で示す。$`\rho(x)`$はテント写像$`(1)`$のヒストグラムを正規化することで求めた。ヒストグラム作成に使ったコードは[./tent_invariant_dist.py](./tent_invariant_dist.py)である。
