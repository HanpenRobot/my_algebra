...
def draw_return_map(init_value: float, n: int):
    ansX = [init_value]
    tmp_X = init_value
    X = func(x=tmp_X)
    ansX.append(X)
    ...
    tmp_graph_Y = [func(x=x) for x in tmp_arry]
    plt.plot(  # plot tent map
        tmp_arry,
        tmp_graph_Y,
        label=f"$y=D(x)$",
...
def draw(n: int):
    ...
    plt.xlim(0, AXIS_MAX)
    plt.ylim(0, AXIS_MAX)
    draw_return_map(init_value=INIT_VALUE, n=n)
...
res.save(f"double_func_return_map_init_{INIT_VALUE}.gif", writer="imagemagick")
