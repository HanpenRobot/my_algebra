...
> である。$`(x_0:x_1)`$は
> ```math
> (x_0:x_1) := \{  (\alpha x_0,\alpha x_1) | \alpha \in R^1 \}
> ```
> と表せる。つまり、$`(x_0:x_1)`$は比
> ```math
> (x_0:x_1)=\left(1:\frac{x_1}{x_0}\right)=\left(\frac{x_0}{x_1}:1\right)=(2 x_0:2 x_1)=...
> ```
> である。
> ※$`(x_0:x_1)`$が原点$`(0:0)`$を通る直線


> [!IMPORTANT]
> 1次元射影直線$`RP^1`$の要素は**座標**ではなく、**原点を通る直線**であることに要注意 !
> ```math
> RP^1 \ni 原点を通る直線
> ```
